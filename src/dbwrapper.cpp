...
long TxDBProcess::Select(leveldb::DB *db, char *Key, std::vector<std::string> &values, long newest_num)
{
	...
	char *RecMax = (char *)malloc(17 * sizeof(char));
	sprintf(RecMax, "%d", recmax);
	char * left0 = (char *)malloc(34 * sizeof(char));
	strcpy(left0, (const char *)"0000000000000000");
	char *tmpstr = (char *)malloc(33 * sizeof(char));
	strcat(left0, RecMax);
	strcpy(tmpstr, (const char*)(left0 + strlen(RecMax)));
	strcpy(KeyIndex, Key);
	strcat(KeyIndex, tmpstr);
	free(left0); left0 = NULL;
	free(tmpstr); tmpstr = NULL;
	free(RecMax); RecMax = NULL;
	*/
	char * KeyIndex = (char *)malloc(257 * sizeof(char));
...
long TxDBProcess::Select(leveldb::DB *db, char *Key, std::vector<std::string> &values, long newest_num, char *Value)
{
	...
	int i = 0;
	long readsum = recsum < newest_num ? recsum : newest_num;
...
/*删除有重复的txid的函数*/
/*
int TxDBProcess::DeleteTx(leveldb::DB *db, char *address, char* txid)
{

long max = GetMax(db, address);
long sum = GetSum(db, address);
if (sum == 0 || max == 0)
return 0;
vector<string> Key_ToDel;
leveldb::Iterator* it = db->NewIterator(leveldb::ReadOptions());
int i = 0;
for (it->Seek(address); it->Valid()&&i<=sum; it->Next())
{
if (strcmp(it->value().ToString().c_str(), txid) == 0)
{
Key_ToDel.push_back(it->key().ToString());
}
...
if (Key_ToDel.size() == 1 && sum == 1)
{
s = db->Delete(leveldb::WriteOptions(), Key_ToDel.at(0));
if (!s.ok())
return -1;
s = db->Delete(leveldb::WriteOptions(), address);
if (!s.ok())
return -1;
}
for (int ii = 0; ii < Key_ToDel.size(); ii++)
{
s = db->Delete(leveldb::WriteOptions(), Key_ToDel.at(ii));
if (!s.ok())
return -1;
}

char *getkeyinfo = (char *)malloc(129 * sizeof(char));
char *maxstr = (char *)malloc(17 * sizeof(char));
char *pmax = maxstr;
char *sumstr = (char *)malloc(17 * sizeof(char));
char *Key_Value = (char *)malloc(34 * sizeof(char));

sum = sum - Key_ToDel.size();

leveldb::Iterator* itor = db->NewIterator(leveldb::ReadOptions());
int j = 0;

itor->Seek(address);
while (j < sum && itor->Valid())
{
...
strcpy(getkeyinfo, (const char*)(itor->key().ToString().c_str()));

delete itor;

maxstr = getkeyinfo + (strlen(getkeyinfo) - 16);

sscanf(maxstr, "%d", &max);
sprintf(sumstr, "%d", sum);
sprintf(maxstr, "%d", max);
strcpy(Key_Value, sumstr);
strcat(Key_Value,(const char*)"-");
strcat(Key_Value, maxstr);
s = db->Put(leveldb::WriteOptions(), address, Key_Value);
if (!s.ok())
{
...
/*
bool TxDBProcess::GetEndKey(leveldb::DB *db, char *Key, char *EndKey)
{
int max;
max = GetMax(db, Key);
if (max == 0)
return false;
char *maxstr = (char *)malloc(17 * sizeof(char));
char *fill0 = (char *)"0000000000000000";
char * tmpstr = (char *)malloc(129 * sizeof(char));
sprintf(maxstr, "%d", max);

strcpy(tmpstr, (const char*)fill0);
strcat(tmpstr, (const char*)maxstr);
strcpy(EndKey, (const char*)Key);
strcat(EndKey, (const char*)(tmpstr + strlen(maxstr)));
return true;
}
*/
...
// 	memset(TxInfoStr, '0', sizeof(TxInfoStr));
...
bool TxDBProcess::Select(leveldb::DB *db, char *Key, std::vector<CTxaddressData> &TxInfo)
{
	...
	//char *AddressStr = (char *)malloc(64 * sizeof(char));
	char *nTypeStr = (char *)malloc(2 * sizeof(char));
	char *amountStr = (char *)malloc(33 * sizeof(char));
	s = db->Get(leveldb::ReadOptions(), Key, &txinfostr);
	//LogPrintfQ("Select===== txinfostr : %s \n",txinfostr);
	if (!s.ok())
	{
	//	free(AddressStr); AddressStr = NULL;
		free(nTypeStr); nTypeStr = NULL;
		free(amountStr); amountStr = NULL;
		return false;
	}
	...
	while (pos != -1)
	{
		...
		//LogPrintfQ("pos= %d ,str1 = %s \n", pos, str1);
		//strncpy(amountStr, (const char *)txinfostr.c_str(), pos);
		sscanf((char*)str1.c_str(), "%lld", &ctad.amount);
		//LogPrintfQ("amount = %lld \n", ctad.amount);
		txinfostr = txinfostr.substr(pos + 1);
...
// 	{
// 
// 		// look for the postion of first ","
// 		pos = txinfostr.find(',');
// 
// 		// substring for struct member address
// 		ctad.address = txinfostr.substr(0, pos);
// 
// 		//substring right back left department
// 		txinfostr = txinfostr.substr(pos + 1, strlen(txinfostr.c_str()) - pos - 1);
// 
// 		//look for the postion of second ","
// 		pos = txinfostr.find(',');
// 
// 		//substring nTypeStr for struct member ntpye;
// 		strncpy(nTypeStr, (const char*)txinfostr.c_str(), pos);
// 
// 		//convert string to int of ntype;
// 		sscanf(nTypeStr, "%d", &ctad.ntype);
// 
// 		//substring right back left department
// 		txinfostr = txinfostr.substr(pos + 1, strlen(txinfostr.c_str()) - pos - 1);
// 
// 		//look for the first "@" of the string txinfostr;
// 		pos = txinfostr.find('@');
// 
// 		//copy the string for strcut member amount
// 		strncpy(amountStr, (const char *)txinfostr.c_str(), pos);
// 
// 		//convert string to longlong of amount
// 		sscanf(amountStr, "%lld", &ctad.amount);
// 
// 		//push_back the struct ctad to vector TxInfo
// 		TxInfo.push_back(ctad);
// 
// 		//substring right back left department
// 		txinfostr = txinfostr.substr(pos + 1, strlen(txinfostr.c_str()) - pos - 1);
// 
// 		//look for the postion of next first ","
// 		pos = txinfostr.find(",");
// 
// 	}
// 
