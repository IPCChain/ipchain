    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
				...
				//add begin by li at 2017-10-13 11:51 
				//解决rpc命令-addnode，程序关闭后再启动，仍然需要-addnode的问题
				//CNetAddr local;
				//LookupHost("127.0.0.1", local, false);
				//CAddress addr(service, NODE_NONE);
				//addr.nTime = GetTime()- 24 * 3600;
				//addrman.Add(addr,local, 2 * 60 * 60);
		
				AddNewAddress(CAddress(service, NODE_NONE), CAddress(service, NODE_NONE), 2 * 60 * 60);
				DumpAddresses();
				//add end by li
            }
        }
