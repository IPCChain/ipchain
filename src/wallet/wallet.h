...
public:
	...
	void GetAmounts2(std::list<COutputEntry>& listReceived,
		std::list<COutputEntry>& listSent, CAmount& nFee, std::string& strSentAccount, const isminefilter& filter) const;//IPC zhaolingbujisuan
	...
	void GetAmountsTokenForGetttx(std::list<COutputEntryToken>& listReceived,
		std::list<COutputEntryToken>& listSent, CAmount& nFee, std::string& strSentAccount, const isminefilter& filter) const;//Token
...
public:
    ...
    bool AddMultiAddress(std::string strmultiscript, std::string& strFailReason,int &nRequired,int &nmembers, std::string strAccount = "",bool brpc = false);
    ...
    void UnionCWalletTxFromAddress(std::map<uint256, const CWalletTx*>& vCoins,std::string address);
    ...
    bool SignUnionTxWithSigns(const std::string& strtx, std::string& strFailReason, ScriptError& serror, std::string& strtxout,CMutableTransaction& mergedTx,std::vector<std::string>& signtxs);
...
public:
	...
	void AvailableIPCCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue = false) const;
	void AvailableTokenCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue = false) const;
	void AvailableUnionCoins2(CScript& strunionaddress, std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue = false) const;
	//
	bool GetSymbolbalance(std::string& tokensymbol, uint64_t& value/*, uint64_t nconfirmed = DEFAULT_TX_CONFIRM_TARGET*/);
	void ListTokenBalance(std::map<std::string, uint64_t>& TokenList);
	uint8_t GetAccuracyBySymbol(std::string& tokensymbol);
	uint32_t GetIssueDateBySymbol(std::string& tokensymbol);
    uint128 GetHashBySymbol(std::string& tokensymbol);
    ...
    bool analysisMultiTransaction(const std::string& strtx,std::string& strFailReason,\
            std::string& address,CAmount& money,int haveSigned,std::string& sourceaddress);
    bool CommitMultiTransaction(CMutableTransaction& mergedTx,std::string& error);
    bool SignMultiTransaction(const std::string& strtx, std::string& strFailReason, ScriptError& serror,std::string& strtxout,CMutableTransaction& mergedTx,bool isrpc=false);
    bool P2SHProduceSignature(CMutableTransaction& mergedTx,ScriptError& serror);
    bool checkMyUnion(const CTransaction& tx);
    void getUnionAddresses(std::vector<UnionAddressInfo>& addressbook);
    int  getUnionAddressLocalTime(std::string address);
    void AvailableUnionCoins(std::map<std::string,CAmount>& moneynums, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, bool fIncludeZeroValue=false) const;
