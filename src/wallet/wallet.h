...
//! -paytxfee default
static const CAmount DEFAULT_TRANSACTION_FEE = 100000; //默认为0,001 -- ipc 20171107
//! -fallbackfee default
...
enum WalletFeature
{
    ...
    FEATURE_WALLETCRYPT = 40000, // wallet encryption 钱包加密
    FEATURE_COMPRPUBKEY = 60000, // compressed public keys 压缩性公钥

    FEATURE_HD = 130000, // Hierarchical key derivation after BIP32 (HD Wallet)
    FEATURE_LATEST = FEATURE_COMPRPUBKEY // HD is optional, use FEATURE_COMPRPUBKEY as latest version HD是可选的，用FEATURE_COMPRPUBKEY作为最新版本
};


/** A key pool entry 密钥池条目*/
class CKeyPool
{
public:
    int64_t nTime;
...
struct CRecipient 
{
    ...
    CAmount nAmount;			//总额
    bool fSubtractFeeFromAmount; //是否从Amount减去交易费
};
...
/** A transaction with a merkle branch linking it to the block chain.与merkle分支的交易将其链接到块链 */
class CMerkleTx
{
private:
  /** Constant used in hashBlock to indicate tx has been abandoned 在区块中使用的常量来表示tx已被放弃*/
...
public:
    ...
    bool hashUnset() const { return (hashBlock.IsNull() || hashBlock == ABANDON_HASH); }  //判断hash是否为未设置过
...
/** 
 * A transaction with a bunch of additional info that only the owner cares about.一个交易与一大堆附加信息，只有业主关心
 * It includes any unrecorded transactions needed to link it back to the block chain.它包括将其链接回块链所需的任何未记录的交易
 */
 ...
 * Database key is acentry<account><counter>.数据库密钥是ancestry <account> <counter>。
 */
 ...
 *CWallet是密钥库的扩展，它还维护一组事务和余额，并提供创建新事务的能力。
 */
...
private:
	...
	//bool SelectIPCCoins(const std::vector<COutput>& vAvailableCoins, const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl *coinControl = NULL) const;
    ...
    int64_t nNextResend;
    int64_t nLastResend;
    bool fBroadcastTransactions; //广播交易标志
     ...
     */
	//用于跟踪有用的outpoint，并检测和报告冲突（双重花费或突变的交易，其中突变体被开采）。
...
public:
    ...
    std::map<uint256, CWalletTx> mapWallet; //钱包中的交易集合
    ...
    const CWalletTx* GetWalletTx(const uint256& hash) const;	//得到钱包中指定交易哈希的交易细节

	std::map <std::string, uint64_t> TokenValueMap; //各种代币所对应的余额  ---返回列表给钱包
	std::vector<std::string> TokensymbolList;			//钱包中代币symbol的集合
	SecureString curstrWalletPassphrase;				//当前钱包密码
    //! check whether we are allowed to upgrade (or already support) to the named feature
	/*比较钱包版本，判断是否支持指定特点*/
    bool CanSupportFeature(enum WalletFeature wf) { AssertLockHeld(cs_wallet); return nWalletMaxVersion >= wf; }

    /**
     * populate vCoins with vector of available COutputs.用可用的COutputs的向量填充vCoins。
     */
	...
	//
	bool GetSymbolbalance(std::string& tokensymbol, uint64_t& value);
    ...
    bool EncryptWallet(const SecureString& strWalletPassphrase); //加密钱包
	...
	bool CreateTokenRegTransaction(std::string& strReglabel, const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut,
		std::string& strFailReason, const CCoinControl *coinControl = NULL, bool sign = true);
	//创建代币交易
	bool CreateTokenTransaction(/*std::string& txid, int Index*/std::string& tokensymbol, uint64_t TokenValue, const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut,
		std::string& strFailReason, const CCoinControl *coinControl = NULL, bool sign = true);
	//end
	...
	//提交共识类交易接口，在本接口中调用dpoc层的加入与退出接口
	...
	//bool CommitExitCampaignTransaction(CWalletTx& wtxNew, CReserveKey& reservekey, CConnman* connman, CValidationState& state);
	//获取共识奖励列表。目前只返回时间和奖励金额。该接口形式随后续要求调整
	...
	////判断一个交易的vout是否是指向系统账户的
	//bool IsSystemAccounts(const CTxIn& txin) const;
	//bool IsSystemAccounts(const CTxOut& txout) const;
	//bool IsSystemAccounts(const CTransaction& tx) const;
...
/** A key allocated from the key pool.从密钥池分配的密钥 */
...
{
    // Fill in dummy signatures for fee calculation.在费用计算中填入假签名。
	int nIn = 0;
	if (nseek > 0)
		nIn = nseek;
