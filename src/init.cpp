...
bool bNeedReloadBlockData = false;
#if ENABLE_ZMQ
...
//flush blockinex data to disk 
static boost::thread *th1;
static bool th1_stop =false;
//////////////////////////////////////////////////////////////////////////////
//
// Shutdown
//
...
void Shutdown()
{
    ...
    LogPrintf("Shutdown(): before if (pIPCCheckMaps != NULL)\n");
    {
		if (pIPCCheckMaps != NULL)
		{
			delete(pIPCCheckMaps);
		}
		pIPCCheckMaps = NULL;
        ...
        LOCK(cs_main);
        if (pcoinsTip != NULL) {
            LogPrintf("Shutdown(): before FlushStateToDisk();\n");
            FlushStateToDisk();
            LogPrintf("Shutdown(): after FlushStateToDisk();\n");
        }
        delete pcoinsTip;
        pcoinsTip = NULL;
...
#endif
       LogPrintf("Shutdown(): before #ifndef WIN32\n");
#ifndef WIN32
    try {
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    LogPrintf("end threadGroup.create_thread\n");
}
...
bool AppInitParameterInteraction()
{
 ...
 //zou mark                   UpdateRegtestBIP9Parameters(Consensus::DeploymentPos(j), nStartTime, nTimeout);
                    found = true;
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single ipcchain process is using the data directory.
...
static void FlushMapBlockIndex()
{
	CBlockIndex::m_current_time = 0;

	while (th1_stop == false)
	{
		boost::this_thread::sleep_for(boost::chrono::milliseconds(1000));
		CBlockIndex::m_current_time ++;
		//if (CBlockIndex::m_current_time % 60 == 0)
		if (CBlockIndex::m_current_time % 10 == 0)
		{
			boost::mutex::scoped_lock lock(CBlockIndex::m_mu);
			//std::cout << "-----" << std::endl;
			BlockMap::iterator it = mapBlockIndex.begin();
			while (it != mapBlockIndex.end())
			{
				if (CBlockIndex::m_current_time - it->second->m_last_time > 5)
				{
					//std::cout << "+++" << std::endl;;
					it->second->flush ();
				}
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    while (!fLoaded) {
        ...
        do {
            try {
				...
				delete pIPCCheckMaps;
				pIPCCheckMaps = new IPCCheckMaps();

				delete pTxDB;
				pTxDB = new TxDBProcess();
				if (fAddressIndex)
					pTxDB->Init();
				...
				CBlockIndex::InitCBlockIndexGlobal(); //mgwang
				//std::cout << "LoadBlockIndexstart: " << get_rmem(getpid()) << std::endl;
				LogPrintf("[INIT] begin LoadBlockIndex\n");
                if (!LoadBlockIndex(chainparams)) {
                    strLoadError = _("Error loading block database");
                    break;
                }
				//std::cout << "LoadBlockIndexend: " << get_rmem(getpid()) << std::endl;
				...
				LogPrintf("[INIT] uiInterface.InitMessage(_(\"Verifying blocks...\"))\n");
				...
				LogPrintf("[INIT] prepare CVerifyDB().VerifyDB\n");
				...
				LogPrintf("[INIT] Start the time server\n");
				timeService.start();

				LogPrintf("[INIT] Analyze local snapshots\n");
				struct timeval begintime;
				gettimeofday(&begintime, NULL);
				//std::cout << "analysisConsensusSnapshotsstart: " << get_rmem(getpid()) << std::endl;
				CConsensusAccountPool::Instance().analysisConsensusSnapshots();
				//std::cout << "analysisConsensusSnapshotsend: " << get_rmem(getpid()) << std::endl;
				//Set consensus
				...
				if (CDpocInfo::Instance().GetLocalAccount(strPublicKey))
				{
					...
					CDpocInfo::Instance().setLocalAccoutVar(strPublicKey);
				}

				struct timeval finishtime;
				gettimeofday(&finishtime, NULL);
				LogPrintf("[INIT] Analyzing local blocks takes time %d seconds \n", finishtime.tv_sec - begintime.tv_sec);

				LogPrintf("[INIT] Start the meeting thread\n");
				CDpocMining::Instance().start();
				//------------------------------------dpoc end


				LogPrintf("[INIT] CVerifyDB().LoadICMFromDisk\n");
				//Change to read local disk data into memory
				tokenDataMap.clear(); 
				CVerifyDB().LoadICMFromDisk();
	...
	if (IsArgSet("-pswd")) {
		pwalletMain->curstrWalletPassphrase.reserve(100);
		pwalletMain->curstrWalletPassphrase = GetArg("-pswd", "").c_str();
	}
    ...
    // Wait for genesis block to be processed
    {
        ...
        while (!fHaveGenesis&&condvar_GenesisWait.wait_for(lock,boost::chrono::seconds(3)) == boost::cv_status::timeout) {
                if(fRequestShutdown){
                    LogPrintf("fRequestShutdown,condvar_GenesisWait return)\n");
                    return false;
                }
            }
...
#endif
	th1 = new boost::thread (FlushMapBlockIndex);
	return !fRequestShutdown;
}
