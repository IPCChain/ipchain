    ...
    while (!fLoaded) {
        ...
        do {
            try {
				...
				LogPrintf("[INIT] 准备调用LoadBlockIndex\n");
                if (!LoadBlockIndex(chainparams)) {
                    strLoadError = _("Error loading block database");
                    break;
                }

				//如果被加载的链发生了错误的起源，立刻就会被退出
				...
				LogPrintf("[INIT] 准备调用CVerifyDB().VerifyDB\n");
				...
				LogPrintf("[INIT] 启动时间服务器\n");
				//------------------dpoc begin
				timeService.start();

				LogPrintf("[INIT] 分析本地快照\n");
				...
				LogPrintf("[INIT] 分析本地块耗费%d秒\n", finishtime.tv_sec - begintime.tv_sec);

				LogPrintf("[INIT] 启动会议线程\n");
				CDpocMining::Instance().start();
				//------------------------------------dpoc end


				LogPrintf("[INIT] 准备调用CVerifyDB().LoadICMFromDisk\n");
				//在启动时执行遍历全链以缓存Unique全局约束和添加所有块上交易到缓存列表的工作
				//更改为读取本地磁盘数据到内存中
				tokenDataMap.clear(); //代币数据缓存清空
				CVerifyDB().LoadICMFromDisk();
			

// 				if (fAddressIndex)
// 				{
// 					if (!CVerifyDB().CacheDataNeeded(chainparams, pcoinsdbview)) {
// 						strLoadError = _("Catch Unique Limition or tx datas failed");
// 						break;
// 					}
//			     }
