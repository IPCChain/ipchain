...
static const int64_t MEMORY_ALLOCATION_BIG = 129;
static const int64_t MEMORY_ALLOCATION_MID = 65;
static const int64_t MEMORY_ALLOCATION_MIN = 33;

class CTxaddressData
{
public:
	std::string  address;	//来源或输出地址
	int ntype;				//0:输出  1：输入
	int64_t amount;			//金额  +：out  -：in

	CTxaddressData(){}
	CTxaddressData(std::string& str, int type, int64_t nvalue){
		address = str;
		ntype = type;
		amount = nvalue;
	}
...
public:
	struct KeyInfo{
		...
		char StartKey[MEMORY_ALLOCATION_MAX];   /* key序列的起始key */
		char EndKey[MEMORY_ALLOCATION_MAX];     /* keyx序列的结束key */
		int64_t RecordSum;                        /* Key序列的记录总数 */
		int64_t KeyHight;
	};
