    ...
    ui(new Ui::setdialog)
{
    ...
    setButtonPic(NULL);
    ui->label_6->setText(QString::fromStdString(IPCHAIN_VERSION));
...
void setdialog::on_pushButton_addressbook_pressed()
{
    ...
    Q_EMIT openAddBookPagewidget(5);
}
...
{
    setButtonPic(ui->pushButton_pic_addressbook);
    if(walletStackBranchPage->indexOf(Addbookpage)==-1)
...
{
    if(walletStackBranchPage->indexOf(PasswordSettingPage)==-1)
