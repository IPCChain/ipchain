    ...
    ui(new Ui::TallyApply)
{
    ...
    {
        ui->coinEdit->setText("20");
    }else
    {
        ui->coinEdit->setText("10000");
    }
...
void TallyApply::on_pushButton_apply_pressed()
{
    ...
    {
        LOG_WRITE(LOG_INFO,"tallyapply","getOptionsModel");
        return;
    }
    ...
    int64_t y = t.toInt();
    QString remainder_str;// = QString::number(y).rightJustified(8, '0');
    QString quotient_str = QString::number(y);
    remainder_str = QString::number(y)+"00000000";

    recipient.address = ui->pushButton_address->text();//ui->addEdit->text(); //"145HXcDtMeqbYLrJpPCcWmvDnvKpjUkb6Z";// //"1GodKq4aFhxFUG5Ctr81chJqo8BM9Zwhz";//address
    recipient.label = "";//biaoqian
    recipient.amount = remainder_str.toDouble();//y ;//56789 ;//
    recipient.message = "";
    recipient.fSubtractFeeFromAmount = true;
    recipients.append(recipient);

    QString add1 = ui->pushButton_address->text();//ui->addEdit->text();
