...
void IpcRegister::on_pushButton_next_pressed()
{
    ...
    std::cout<<name.toStdString().length()<<std::endl;
    ...
    else
    {
        if(msg == "AmountExceedsBalance")
            ...
            ui->label_errmsg->setText(tr(" ")+msg);//tr("Other")+
    }
...
void IpcRegister::on_pushButton_browse_pressed()
{
    ...
    boost::thread m_ptr(fileToMd5Thread,file_full.toStdString(),string("IpcRegister"),(void*)this);
        m_ptr.detach();
}
