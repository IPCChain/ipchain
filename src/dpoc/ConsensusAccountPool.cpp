...
CConsensusAccountPool::CConsensusAccountPool()
{
	...
	m_strSnapshotDir = std::string("Snapshots");
...
bool  CConsensusAccountPool::checkNewBlock(const std::shared_ptr<const CBlock> pblock, uint160 &pubkeyHash, uint32_t blockHeight, DPOC_errtype &errorType)
{
	...
	uint64_t meetingstarttime = 0;
	if (!bReboot)
	{
		meetingstarttime = meetingStartSnapshot.meetingstoptime;
	}
	else
	{
		meetingstarttime = pblock->nPeriodStartTime;
		LogPrintf("[CConsensusAccountPool::GetTimeoutIndexs] reboot meetingstarttime =%d", pblock->nPeriodStartTime);
		bReboot = false;
	}
	//bool result = CDpocMining::Instance().GetMeetingList(meetingstarttime, consensusList);
	//if (result)
	if (CDpocMining::Instance().GetMeetingList(meetingstarttime, consensusList))
	{
		...
		LogPrintf("[CConsensusAccountPool::GetTimeoutIndexs] meetingstarttime=%d blockstartime=%d meeting pockager sort：\n", meetingstarttime, pblock->nPeriodStartTime);
    ...
    LogPrintf("[CConsensusAccountPool::checkNewBlock] end true");
	return true;
}
...
bool CConsensusAccountPool::verifyDPOCBlock(const std::shared_ptr<const CBlock> pblock, uint32_t blockHeight, DPOC_errtype &rejectreason)
{
	...
	LogPrintf("[CConsensusAccountPool::verifyDPOCBlock()]  pubicKey160hash=%s\n", pubicKey160hash.GetHex());
	...
	if (GetLastSnapshot(lastSnapshot))
	{
		...
		for (const auto& tx : pblock->vtx)
		{
			if (tx->IsCoinBase())
			{
				...
				{
					if ((txout.txType != TXOUT_NORMAL) && (txout.txType != TXOUT_CAMPAIGN))
					{
						LogPrintf("[CConsensusAccountPool::verifyDPOCBlock()] coinbase TX has wrong Type reurn false and MisbeHaving\n");
...
bool CConsensusAccountPool::ContainPK(uint160 pk, uint16_t& index)
{
	for (int i = 0; i < candidatelist.size(); i++) {
		//LogPrintf("[CConsensusAccountPool::ContainPK] i = %d ,PK=%s",i, candidatelist.at(i).getPubicKey160hash().GetHex());
...
bool CConsensusAccountPool::IsAviableUTXO(const uint256 hash)
{
	...
	for (searchIt = cachedSnapshots.begin(); searchIt != cachedSnapshots.end(); searchIt++)
	{
		...
		{
            if (*refundit <= candidatelist.size() && candidatelist[*refundit].getTxhash() == hash)
			return false;
		}
...
bool CConsensusAccountPool::popDPOCBlock( uint32_t blockHeight)
{
	...
	uint32_t u32OldHeight = 0;
	std::map<uint32_t, SnapshotClass>::iterator iterSnapshot = snapshotlist.end();
	if (iterSnapshot != snapshotlist.begin())
	{
		iterSnapshot--;
		u32OldHeight = iterSnapshot->first;
	}
	else
	{
		return false;
	...
	if (bUpdate)
	{
		...
		truncateSnapshotFile(u32OldHeight, blockHeight);
	}
...
bool CConsensusAccountPool::PushSnapshot(SnapshotClass snapshot)
{
	...
	boost::filesystem::path curTargetDir = GetDataDir();
	...
	if ((0 == (snapshot.blockHeight % SNAPSHOTINSERT)) && (snapshot.blockHeight != 0))
	{
		writeCandidatelistToFile();

		//The initial block height to be stored
		int  nPushBeginHeight = 0;
		if (snapshot.blockHeight == SNAPSHOTINSERT)
		{
			nPushBeginHeight = 0;
		}
		else
		{
			nPushBeginHeight = snapshot.blockHeight - SNAPSHOTINSERT;
			++nPushBeginHeight;
		}

		int nFileNum = 0;
		nFileNum = nPushBeginHeight / SNAPSHOTLENGTH;

		setSnapshotFilePath(nFileNum);
		
		boost::filesystem::path Shapshots = curTargetDir / m_strSnapshotDir;
		if (!boost::filesystem::exists(Shapshots))
		{
			boost::filesystem::create_directories(Shapshots);
		}

		boost::filesystem::path pathTmp = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
		uint64_t fileSize = 0;
		...
		boost::filesystem::path pathTmpSnapshot = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
		FILE *fileSnapshot = fopen(pathTmpSnapshot.string().c_str(), "ab+");
		...
		//Open the snapshot file
		boost::filesystem::path pathTmpIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
		...
		//File starting height
		std::map<uint32_t, SnapshotClass>::iterator iter = snapshotlist.find(nPushBeginHeight);
		
		for (; iter != snapshotlist.end(); ++iter)
		{
			//Write a snapshot
			CAutoFile fileout(fileSnapshot, SER_DISK, CLIENT_VERSION);
			if (fileout.IsNull())
			{
			...
			CAutoFile fileoutIndex(fileIndex, SER_DISK, CLIENT_VERSION);
			if (fileoutIndex.IsNull())
			{
				LogPrintf("CSerializeDpoc::WriteToDisk return false by fileout.IsNull() \n");
				fclose(fileIndex);
				return false;
...
bool  CConsensusAccountPool::analysisConsensusSnapshots()
{
	...
	int nAllSnapshotSize = 0;
	{
		...
		while (true)
		{
			setSnapshotFilePath(nTargetIndex);

			boost::filesystem::path pathIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
			boost::filesystem::path pathSnapshot = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
			if (!boost::filesystem::exists(pathIndex) || !boost::filesystem::exists(pathSnapshot))
			{
				LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] Get Snposhotfile Index = %d \n",nTargetIndex);
				break;
			}
			++nTargetIndex;
		}
		...
		int nIndex = (nTargetIndex-2) ?  (nTargetIndex-2):0;
		while (true)
		{	
			setSnapshotFilePath(nIndex);

			boost::filesystem::path pathIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
			boost::filesystem::path pathSnapshot = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
			if (!boost::filesystem::exists(pathIndex) || !boost::filesystem::exists(pathSnapshot))
			{
				LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] SnapshotFile is null \n");
				break;
			}
			else
			{
				uint64_t u64fileSizeSnapshot = boost::filesystem::file_size(pathSnapshot);
				uint64_t u64FileSizeIndex = boost::filesystem::file_size(pathIndex);
				if ((u64fileSizeSnapshot > 0)&&(u64FileSizeIndex >= m_u64SnapshotIndexSize))
				{
					for (uint64_t nBegin = 0; nBegin <= u64FileSizeIndex - m_u64SnapshotIndexSize; nBegin += m_u64SnapshotIndexSize)
					{
						CSerializeDpoc<CSnapshotIndex> serializeIndex;
						CSnapshotIndex  sanpshotIndex;
						serializeIndex.ReadFromDiskS(sanpshotIndex, nBegin, m_strSnapshotIndexPath);
						
						//Load the snapshot
						if (sanpshotIndex.nOffset < u64fileSizeSnapshot)
						{
							CSerializeDpoc<SnapshotClass> serializeSnapshot;
							SnapshotClass snapshot;
							serializeSnapshot.ReadFromDiskS(snapshot, sanpshotIndex.nOffset, m_strSnapshotPath);
							...
							if ((sanpshotIndex.nOffset+getSnapshotSize(snapshot)) >= u64fileSizeSnapshot)
							{
								break;
							}
						}
		...
		LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] loop.num  ：%d\n", nAllSnapshotSize);
		if (1 < snapshotlist.size())
		{
			std::map<uint32_t, SnapshotClass>::iterator iterMap = snapshotlist.end();
			
			LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] snapshotlist nAllSnapshotSize ：%d\n", nAllSnapshotSize);
			--iterMap;
	...
	{
		if ((nChainHeight == -1)||(nChainHeight < 0))
		{
			LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] nChainHeight == -1 and remove all snapshot file \n");
			int nIndexTmp = 0;
			while (true)
			{
				setSnapshotFilePath(nIndexTmp);
				...
				{
					if (boost::filesystem::exists(pathIndex))
					{
						boost::filesystem::remove(pathIndex);
					}

					if (boost::filesystem::exists(pathSnapshot))
					{
						boost::filesystem::remove(pathSnapshot);
					}
				}
			...
			candidatelist.clear();
			//writeCandidatelistToFile();
			boost::filesystem::path pathTmpList = curTargetDir / m_strCandidatelistPath;
			if (boost::filesystem::exists(pathTmpList))
			{
				boost::filesystem::remove(pathTmpList);
			}
		}
		else
		{
			LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] popDPOCBlock is called\n");
			popDPOCBlock(nChainHeight);
		}
	...
	{
		LogPrintf("[CConsensusAccountPool::analysisConsensusSnapshots] nChainHeight >= SnapshotSize is called\n");
...
void CConsensusAccountPool::writeCandidatelistToFile()
{
	...
	FILE *fileList = fopen(pathTmp.string().c_str(), "ab+");
	if (fileList == NULL)
	{
		LogPrintf("[CConsensusAccountPool::writeCandidatelistToFile] fileIndex is NULL\n");
		return ;
	}
	...
	{
		CAutoFile fileout(fileList, SER_DISK, CLIENT_VERSION);
		if (fileout.IsNull())
		{
			LogPrintf("CSerializeDpoc::WriteToDisk return false by fileout.IsNull() \n");
			fclose(fileList);
		...
		try {
			...
			LogPrintf("[CConsensusAccountPool::writeCandidatelistToFile] return false by %d \n", e.what());
			return ;
		}

		fileList = fileout.release();
	}	
...
{
	boost::filesystem::path pathTmp = GetDataDir() / m_strSnapshotDir / m_strSnapshotPath;
	...
	boost::filesystem::path pathTmpIndex = GetDataDir() / m_strSnapshotDir / m_strSnapshotIndexPath;
	FILE *fileIndex = fopen(pathTmpIndex.string().c_str(), "ab+");
	if (fileIndex != NULL)
	{
		FileCommit(fileIndex);
...
bool CConsensusAccountPool::verifyPkInCandidateListByTime(int64_t curStarttime, CKeyID &pubicKey160hash)
{
	...
	//LogPrintf("[CConsensusAccountPool::verifyPkInCandidateList] Packaging public key Index =%d\n", pkindex);
...
bool CConsensusAccountPool::getCreditbyPkhash(uint160 pkhash, int64_t &n64Credit)
{
	uint16_t tmpIndex;
	if (ContainPK(pkhash, tmpIndex))
	{
		n64Credit = candidatelist[tmpIndex].getCredit();
		return true;
	}
	else
	{
...
bool CConsensusAccountPool::setSnapshotFilePath(int nNum)
{
	std::stringstream sstream;
	sstream << nNum;
	std::string strFileNum = sstream.str();

	m_strSnapshotPath = std::string("Snapshot");
	m_strSnapshotIndexPath = std::string("SnapshotIndex");

	m_strSnapshotPath += strFileNum;
	m_strSnapshotIndexPath += strFileNum;

	return true;

}

bool CConsensusAccountPool::truncateSnapshotFile(uint32_t u32OldHeight, uint32_t u32NewHeight)
{

	boost::filesystem::path curTargetDir = GetDataDir();
	if (u32NewHeight >= u32OldHeight)
	{
		LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] u32NewHeight >= u32OldHeight return false\n");
		return false;
	}
	if (m_u32WriteHeight <= u32NewHeight)
		return true;

	uint32_t u32TruncateNum = 0; 
	int nFileNewNum = (u32NewHeight - 1) / SNAPSHOTLENGTH;
	int nFileOldNum = (u32OldHeight - 1) / SNAPSHOTLENGTH;

	LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] OldHeight = %d,NewHeight = %d,FileNewNum = %d,FileOldNum = %d\n", 
		       u32OldHeight, u32NewHeight, nFileNewNum, nFileOldNum);

	if (nFileNewNum < nFileOldNum)
	{
		for (; nFileOldNum > nFileNewNum; --nFileOldNum)
		{
			setSnapshotFilePath(nFileOldNum);

			boost::filesystem::path pathTmpIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
			if (boost::filesystem::exists(pathTmpIndex))
			{
				boost::filesystem::remove(pathTmpIndex);
				LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] remove SnapshotIndex file num = %d\n", nFileOldNum);
			}
			...
			{
				boost::filesystem::remove(pathTmp);
				LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] remove Snapshot file num = %d\n", nFileOldNum);
			}
		}

		u32TruncateNum = (nFileNewNum + 1)*SNAPSHOTLENGTH - u32NewHeight;
	}
	else
	{
		u32TruncateNum = u32OldHeight - u32NewHeight;
	}

	LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] TruncateNum = %d\n", u32TruncateNum);
	setSnapshotFilePath(nFileNewNum);

	boost::filesystem::path pathTmpIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
	if (boost::filesystem::exists(pathTmpIndex))
	{
		uint64_t u64FileSizeIndex = boost::filesystem::file_size(pathTmpIndex);
		u64FileSizeIndex = u64FileSizeIndex - m_u64SnapshotIndexSize*u32TruncateNum;
				
		FILE *fileIndex = fopen(pathTmpIndex.string().c_str(), "ab+");
		if (fileIndex)
		{
			LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] Truncate SnapshotIndex File \n");
			TruncateFile(fileIndex, u64FileSizeIndex);
		
			FileCommit(fileIndex);
		}
		fclose(fileIndex);
		
		//uint64_t u64FileSizeSnapshot = m_mapSnapshotIndex[nowblockHeight + 1];
		//Truncate the Snapshot file
		boost::filesystem::path pathTmp = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
		//uint64_t fileSize = 0;
		if (boost::filesystem::exists(pathTmp))
		{
			FILE *fileSnapshot = fopen(pathTmp.string().c_str(), "ab+");
			if (fileSnapshot)
			{
				uint64_t u64NewFileSizeSnapshot = m_mapSnapshotIndex[u32NewHeight + 1];
				LogPrintf("[CConsensusAccountPool::truncateSnapshotFile] Truncate Snapshot File \n");
				TruncateFile(fileSnapshot, u64NewFileSizeSnapshot);
...
bool CConsensusAccountPool::splitSnapshotFile()
{
	LogPrintf("[CConsensusAccountPool::splitSnapshotFile] begin\n");

	boost::filesystem::path curTargetDir = GetDataDir();

	boost::filesystem::path pathSnapshotIndexOld = curTargetDir / m_strSnapshotIndexPath;
	boost::filesystem::path pathSnapshotOld = curTargetDir / m_strSnapshotPath;
	if (!boost::filesystem::exists(pathSnapshotIndexOld) || !boost::filesystem::exists(pathSnapshotOld))
	{
		LogPrintf("[CConsensusAccountPool::splitSnapshotFile] no have Snapshot or SnapshotIndex File \n");
		return false;
	}
	
	uint64_t u64fileSizeSnapshot = boost::filesystem::file_size(pathSnapshotOld);
	uint64_t u64FileSizeIndex = boost::filesystem::file_size(pathSnapshotIndexOld);

	if ((u64fileSizeSnapshot > 0) && (u64FileSizeIndex >= m_u64SnapshotIndexSize))
	{
		//Not enough 20001 blocks
		uint64_t nSnapshotNum = u64FileSizeIndex / m_u64SnapshotIndexSize;
		if (nSnapshotNum <= (SNAPSHOTLENGTH+1))
		{
			setSnapshotFilePath(SPLITEDSNAPSHOTFILENOMBER);
			boost::filesystem::path pathSnapshotIndex = curTargetDir / m_strSnapshotDir / m_strSnapshotIndexPath;
			boost::filesystem::path pathSnapshot = curTargetDir / m_strSnapshotDir / m_strSnapshotPath;
			boost::filesystem::copy_file(pathSnapshotIndexOld, pathSnapshotIndex);
			boost::filesystem::copy_file(pathSnapshotOld, pathSnapshot);

			boost::filesystem::remove(pathSnapshotIndexOld);
			boost::filesystem::remove(pathSnapshotOld);
			LogPrintf("[CConsensusAccountPool::splitSnapshotFile] end by true nSnapshotNum<20001\n");
			return true;
		}

		FILE *fileSnapshotIndexOld = fopen(pathSnapshotIndexOld.string().c_str(), "rb");
		if (!fileSnapshotIndexOld)
		{
			LogPrintf("[CConsensusAccountPool::splitSnapshotFile] open fileSnapshotIndexOld error\n");
			return false;
		}

		FILE *fileSnapshotOld = fopen(pathSnapshotOld.string().c_str(), "rb");
		if (!fileSnapshotOld)
		{
			LogPrintf("[CConsensusAccountPool::splitSnapshotFile] open fileSnapshotOld error\n");
			return false;
		}

		int nFileNum = 0;
		uint64_t nSnapshotIndexNewFileSize = 0;
		uint64_t nSnapshotNewFileSize = 0;
		uint64_t nOldSnapshotFileOffset = 0;
		uint64_t nOldSnapshotIndexFileOffset = 0;
		uint64_t nIndex = SNAPSHOTLENGTH+1;

		do
		{
			getSplitedSnapshotAndIndexFileSize("SnapshotIndex", nIndex, nOldSnapshotFileOffset, nOldSnapshotIndexFileOffset,
				                    nSnapshotNewFileSize,  nSnapshotIndexNewFileSize);

			if (!createSplitedSnapshotAndIndexFile(nFileNum, nSnapshotNewFileSize, nSnapshotIndexNewFileSize, fileSnapshotOld, fileSnapshotIndexOld))
			{
				LogPrintf("[CConsensusAccountPool::splitSnapshotFile] createNewSnapshotAndIndexFile return false\n");
				return false;
			}
			
			if (fseek(fileSnapshotIndexOld, nOldSnapshotIndexFileOffset, SEEK_SET)) {
				LogPrintf("CConsensusAccountPool::splitSnapshotFile return false by feek is error \n");
				fclose(fileSnapshotIndexOld);
				return false;
			}

			if (fseek(fileSnapshotOld, nOldSnapshotFileOffset, SEEK_SET)) {
				LogPrintf("CConsensusAccountPool::splitSnapshotFile return false by feek is error \n");
				fclose(fileSnapshotOld);
				return false;
			}

			++nFileNum;

			if (nSnapshotNum - nIndex > SNAPSHOTLENGTH)
			{
				nIndex += SNAPSHOTLENGTH;
			}
			else
			{
				break;
		...
		//last leg
		nSnapshotNewFileSize = u64fileSizeSnapshot - nOldSnapshotFileOffset;
		nSnapshotIndexNewFileSize = u64FileSizeIndex - nOldSnapshotIndexFileOffset;
		if (!createSplitedSnapshotAndIndexFile(nFileNum, nSnapshotNewFileSize, nSnapshotIndexNewFileSize, fileSnapshotOld, fileSnapshotIndexOld))
		{
			LogPrintf("[CConsensusAccountPool::splitSnapshotFile] createNewSnapshotAndIndexFile return false\n");
			return false;
	...
	{
		LogPrintf("[CConsensusAccountPool::splitSnapshotFile] Snapshot or SnapshotIndex File is null\n");
	...
	boost::filesystem::remove(pathSnapshotIndexOld);
	boost::filesystem::remove(pathSnapshotOld);
	LogPrintf("[CConsensusAccountPool::splitSnapshotFile] end\n");
	return true;
}

void CConsensusAccountPool::getSplitedSnapshotAndIndexFileSize(const std::string strSnapshotIndexPath,const uint64_t nIndex,
...
{
	if (nIndex == SNAPSHOTLENGTH + 1)
	{
		nOldSnapshotIndexFileOffset += SNAPSHOTLENGTH * m_u64SnapshotIndexSize;
		nSnapshotIndexNewFileSize = (SNAPSHOTLENGTH + 1) * m_u64SnapshotIndexSize;
	}
	else
	{
		nOldSnapshotIndexFileOffset += SNAPSHOTLENGTH * m_u64SnapshotIndexSize;
		nSnapshotIndexNewFileSize = SNAPSHOTLENGTH* m_u64SnapshotIndexSize;
	}
	
	CSerializeDpoc<CSnapshotIndex> serializeIndex;
	CSnapshotIndex  sanpshotIndex;
	serializeIndex.ReadFromDisk(sanpshotIndex, nOldSnapshotIndexFileOffset, strSnapshotIndexPath);

	nSnapshotNewFileSize = sanpshotIndex.nOffset - nOldSnapshotFileOffset;
	
	nOldSnapshotFileOffset = sanpshotIndex.nOffset;
}

bool CConsensusAccountPool::copyPartofFile(const uint64_t nSize, FILE *fileOld, FILE *fileNew)
{
	if (fileOld && fileNew)
	{
		char *pArrayTemp = new char[nSize]; 
		memset(pArrayTemp, 0, sizeof(char)*nSize);
		if (nSize == fread(pArrayTemp, 1, nSize, fileOld))
		{
			if (nSize == fwrite(pArrayTemp, 1, nSize, fileNew))
			{
				delete [] pArrayTemp;
				pArrayTemp = NULL;
				return true;
...
bool CConsensusAccountPool::createSplitedFile(const std::string strSnapshotPath, const uint64_t nSnapshotNewFileSize, FILE *fileSnapshotOld)
{
	boost::filesystem::path curTargetDir = GetDataDir();

	boost::filesystem::path pathSnapshotL = curTargetDir / m_strSnapshotDir;
	if (!boost::filesystem::exists(pathSnapshotL))
	{
		boost::filesystem::create_directories(pathSnapshotL);
	}
	
	boost::filesystem::path pathSnapshot = curTargetDir / m_strSnapshotDir / strSnapshotPath;
	if (boost::filesystem::exists(pathSnapshot))
	{
		boost::filesystem::remove(pathSnapshot);
	}
	FILE *fileSnapshot = fopen(pathSnapshot.string().c_str(), "ab+");
	if (!fileSnapshot)
	{
		return false;
	}

	if (!copyPartofFile(nSnapshotNewFileSize, fileSnapshotOld, fileSnapshot))
	{
		fclose(fileSnapshot);
		return false;
	}
...
bool CConsensusAccountPool::createSplitedSnapshotIndexFile(std::string strSnapshotIndexPath, const uint64_t nSnapshotIndexNewFileSize, FILE *fileSnapshotIndexOld)
{
	boost::filesystem::path curTargetDir = GetDataDir();

	std::string strSnapshotIndexTempPath = strSnapshotIndexPath;
	strSnapshotIndexTempPath += "Temp";
	if (!createSplitedFile(strSnapshotIndexTempPath, nSnapshotIndexNewFileSize, fileSnapshotIndexOld))
	{
		LogPrintf("[CConsensusAccountPool::splitSnapshotFile] create SplitedSnapshotIndexFile return false\n");
		return false;
	}

	boost::filesystem::path pathSnapshotIndexL = curTargetDir / strSnapshotIndexPath;
	if (boost::filesystem::exists(pathSnapshotIndexL))
	{
		boost::filesystem::remove(pathSnapshotIndexL);
	}
	boost::filesystem::path pathSnapshotIndex = curTargetDir / m_strSnapshotDir / strSnapshotIndexPath;

	FILE *fileIndex = fopen(pathSnapshotIndex.string().c_str(), "ab+");
	if (fileIndex == NULL)
	{
		LogPrintf("[CConsensusAccountPool::createSnapshotIndexFile] fileIndex1 is NULL\n");
		return false;
	}

	boost::filesystem::path pathTmpIndex = curTargetDir / m_strSnapshotDir / strSnapshotIndexTempPath;
	...
	{	
		CSerializeDpoc<CSnapshotIndex> serializeIndex;
		CSnapshotIndex  snapshotIndex;
		serializeIndex.ReadFromDiskS(snapshotIndex, nBegin, strSnapshotIndexTempPath);
		if (nBegin == 0)
		{
			nTemp = snapshotIndex.nOffset;
			snapshotIndex.nOffset = 0;
		}
		else
		{
			//uint64_t nOffset =  sanpshotIndex.nOffset - nTemp;
			//nTemp = sanpshotIndex.nOffset;
			snapshotIndex.nOffset -= nTemp;
		}

		//listIndex.push_back(snapshotIndex);	
		CAutoFile fileoutIndex(fileIndex, SER_DISK, CLIENT_VERSION);
		if (fileoutIndex.IsNull())
		{
			LogPrintf("CSerializeDpoc::WriteToDisk return false by fileout.IsNull() \n");
...
bool CConsensusAccountPool::createSplitedSnapshotAndIndexFile(const int nFileNum,const uint64_t nSnapshotNewFileSize,const uint64_t nSnapshotIndexNewFileSize,
	                                      FILE *fileSnapshotOld, FILE *fileSnapshotIndexOld)
{
	if (nSnapshotNewFileSize == 0 || nSnapshotIndexNewFileSize == 0)
	{
		return false;
	}

	setSnapshotFilePath(nFileNum);
	if (!createSplitedFile(m_strSnapshotPath, nSnapshotNewFileSize, fileSnapshotOld))
	{
		LogPrintf("[CConsensusAccountPool::splitSnapshotFile] create SplitedSnapshotFile return false \n");
		return false;
	}

	if (!createSplitedSnapshotIndexFile(m_strSnapshotIndexPath, nSnapshotIndexNewFileSize, fileSnapshotIndexOld))
	{
		LogPrintf("[CConsensusAccountPool::splitSnapshotFile] create SplitedSnapshotIndexFile return false\n");
		return false;
	}
...
void CConsensusAccountPool::printshopsize()
{
	unsigned int nSnapshotSize = snapshotlist.size();
	unsigned int nSnapshotIndexSize = m_mapSnapshotIndex.size();
	std::cout << "snapshotlist --" << nSnapshotSize << " |m_mapSnapshotIndex -- " << nSnapshotIndexSize << std::endl;
