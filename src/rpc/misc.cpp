...
UniValue getaddresstxids(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() <1 || request.params.size() >3)
		...
		"		\"nCount\"	(numeric)	The num how many txids you want to get\n"  
		"		\"hash\"	(sting)	which txid where you want to select forward\n"
		"      ,...\n"
		"    ]\n"
		"}\n"
		"\nResult:\n"
	...
	if (nCount < NUM_OFDIS)
		nCount = NUM_OFDIS; //这边做下最小显示条目的约束
// 	uint256 hash = 0;
// 	if (request.params.size() >2)
// 		hash.SetHex(request.params[2].get_str());
	std::string hash = "";
	if (request.params.size() >2)
		hash=request.params[2].get_str();

#ifdef ENABLE_TXLEVELDB
	std::vector<std::string> txids;
	txids.clear();
// 
// 	if (!(pTxDB->Select(pTxDB->db, (char*)(straddress.c_str()), txids, 0))) 
// 	{
...
#else
	//****************************** 2017 1121
// 	std::multimap<uint64_t, uint256> txids;
// 	txids.clear();
// 	if (!getmapAddressIndex(straddress, txids))
// 	{
// 		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "this address has no txids");
// 	}
// 	//取到map之后根据uint64_t---height排序 倒着取 先取高度低的 
// 	UniValue result(UniValue::VARR);
// 	bool flag = false;
// 	if (hash == "")
// 		flag = true;	//如果没有输入起始的txid，则默认从最新块开始
// 	std::string txid = "";
// 	//
// 	std::multimap<uint64_t, uint256>::reverse_iterator rit = txids.rbegin();
// 	for (; rit != txids.rend();rit++)
// 	{
// 		txid = rit->second.ToString();
// 		if (!flag && txid == hash)
// 		{
// 			flag = true;
// 			continue;
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() != 1)
		...
		"\nExamples:\n"
		"}\n"
		+ HelpExampleCli("getaddressbalance", "'{[\"balance\" \"received\" ]}'")
		);
	std::string straddress = request.params[0].get_str();
	if (straddress.length() != 36)
	{
...
UniValue getipchashtxids(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() != 1)
		...
		"  \"ipchash\"\n"
		"    [\n"
		"      \"ipchash\"  (string) The uint128 encoded ipchash\n"
		"      ,...\n"
		"    ]\n"
		"}\n"
		"\nResult:\n"
...
// 	txids.clear();
// 	if (!getmapIpchashIndex(ipcHash, txids))
// 	{
...
// 	for (int i = 0; i < txids.size(); i++)
// 	{
// 		std::string txid = txids.at(i).ToString();
// 
// 		result.push_back(txid);
// 	}
	std::vector<std::string> txids;
...
UniValue gettokensymboltxids(const JSONRPCRequest& request)
{
	if (request.fHelp || request.params.size() <1 || request.params.size() >3)
		throw runtime_error(
		"gettokensymboltxids \"tokensymbol\"  (\"nCount\"  \"hash\")\n"
...
// 	std::multimap<uint64_t, uint256> txids;
// 	txids.clear();
// 	if (!getmapTokenSymbolIndex(tokensymbol, txids))
// 	{
// 		throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "this tokensymbol has no txids");
// 	}
// 	//取到map之后根据uint64_t---height排序 倒着取 先取高度低的 
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
	...
	{ "util",				"gettokensymboltxids",	  &gettokensymboltxids,		true,  { "tokensymbol" } },
