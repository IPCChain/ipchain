...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
		...
		const UniValue& VinValue = find_value(o, "value");
		const UniValue& Vinaddress = find_value(o, "address");
	
		if (!VinValue.isNull() || !Vinaddress.isNull())
		{
			if (!VinValue.isNum() || !Vinaddress.isStr())
				throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, vin value must be num");
			nVinValue = AmountFromValue(VinValue);
			const string& addressname = Vinaddress.get_str();
			CBitcoinAddress address(addressname);
			strScriptnewout = GetScriptForDestination(address.Get());
		}
	...
	if (TxType == 4)
	{
		std::cout << " [createrawtransaction ] type =4 ,nVinValue = " << nVinValue << " ,fee= " << fee << std::endl;
		CAmount newoutvalue = nVinValue - totalvalue - AmountFromValue(10) - fee;

		std::cout << " [createrawtransaction ] newoutvalue = " << newoutvalue << std::endl;
	...
	std::cout << "[ createrawtransaction ] Done !"<< std::endl;
...
UniValue signrawtransaction(const JSONRPCRequest& request)
{
	...
	std::cout << " tx.size :" << nBytes << std::endl;
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "rawtransactions",    "createrawtransaction",   &createrawtransaction,   true,  {"inputs","outputs","txtype"} },
