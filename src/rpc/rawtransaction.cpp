...
void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry)
{
			...
			if (fTxIndex)
			{
				...
				switch ((int64_t)prev.txType)
				{
					case 4:
						...
						in.push_back(Pair("tokenvalue", ValueFromTCoins(prev.tokenRegLabel.totalCount,(int) prev.tokenRegLabel.accuracy)));
						in.push_back(Pair("accuracy", prev.tokenRegLabel.accuracy));
						break;
					case 5:
						in.push_back(Pair("tokensymbol", prev.tokenLabel.getTokenSymbol()));
						in.push_back(Pair("tokenvalue", ValueFromTCoins(prev.tokenLabel.value, (int)prev.tokenLabel.accuracy)));
						in.push_back(Pair("accuracy", prev.tokenRegLabel.accuracy));
						break;
					default:
						break;
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
      ...
      out.push_back(Pair("value", ValueFromAmount(txout.nValue)));
		...
		//if (txout.txType == 1)
		//{
		//	uint64_t devoteValue = txout.getDevoteValue();
		//	out.push_back(Pair("devotevalue", devoteValue));
		//}
		switch (txout.txType)
		{
		case 1:
		...
		case 4:
			...
			out.push_back(Pair("TokenTotalCount", ValueFromTCoins(txout.tokenRegLabel.totalCount, (int)txout.tokenRegLabel.accuracy)));
		...
		case 5:
			...
			out.push_back(Pair("TokenValue", ValueFromTCoins(txout.tokenLabel.value, (int)txout.tokenLabel.accuracy)));
...
// 	//测试代码，不应该出现在正式代码中，用来测试tx的新增接口功能的
// 	printf("\n");
// 	printf("tx.getTxType=%u\n", tx->GetTxType());
// 	printf("tx.GetCampaignPublickey = %s\n", tx->GetCampaignPublickey().GetHex().c_str());
// 	printf("tx.GetRegisterIPC = %u\n", tx->GetRegisterIPC());
// 	for (int i = 0; i < tx->vout.size(); i++)
// 	{
// 		printf("  tx.vout[%d].GetCampaignType = %d\n", i, tx->vout[i].GetCampaignType());
// 		printf("  tx.vout[%d].GetCheckBlockContent = %s\n", i, tx->vout[i].GetCheckBlockContent().c_str());
// 	}
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
	...
	//输入参数类型检查。此处参数2即output变成了一个数组，里面放有我们新增加的参数，所以参数2的类型应该变成VARR了
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
		...
		////查找同一笔交易中有没有来自同一个UTXO的输入
		//int k;
		//m = mapInput.find(txid.GetHex());
		//bool founded = false;
		//int count = mapInput.count(txid.GetHex());
		//for (k = 0; k < count; k++, m++)
		//{
		//	if (nOutput == m->second)
		//	{
		...
		//
		//if (founded)
		//	throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, txIn cannot be repeated");
	...
	//重新构造output部分
	set<CBitcoinAddress> setNormalAddress;//对不同的输出类型，地址应该分别加以非重验证处理
	...
	for (unsigned int idx = 0; idx < outputs.size(); idx++) {
		...
		//这里是指如果有data：xxxxx输入的情况下，直接把xxxx压到tx里
		//if (name_ == "data") {
		...
		else
		{
			...
			const UniValue& txTitleObj = find_value(o, "txTitle");

			//IPC交易相关值
			const UniValue& startTimeObj = find_value(o, "starttime");
			const UniValue& stopTimeObj = find_value(o, "stoptime");
			const UniValue& reAuthObj = find_value(o, "reauthorize");
			const UniValue& uniAuthObj = find_value(o, "uniqueauthorize");
			...
			const UniValue& TokenTotalCountObj = find_value(o, "TokenTotalCount");
			...
			case 3://授权交易
				...
				//if (txType == 2)
				//{
				//	if (reAuthObj.get_int() != 1 ||
				//		(ipcLabel.stopTime != 0 && ipcLabel.startTime != 0 && ipcLabel.stopTime < ipcLabel.startTime))
				//		throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, IPC Params is invalid");
				//}
				//else
				//{
				//	if (ipcLabel.stopTime == 0 || ipcLabel.startTime == 0 || ipcLabel.stopTime < ipcLabel.startTime)
				//		throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid parameter, IPC Params is invalid");
				//}

				ipcLabel.hash.SetHex(ipcHashObj.get_str());	
				//对知产交易做输出地址查重
				if (setIPCAddress.count(address))
				{
					bool founded = false;
					multimap<CBitcoinAddress, uint128>::iterator m;
					m = setIPCAddress.find(address);
					for (uint32_t k = 0; k < setIPCAddress.count(address); k++, m++)
					{
						if (m->second == ipcLabel.hash)
						{
							founded = true;
							break;
						}
					...
					if (founded)
						throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, IPC output duplicated address: ") + name_);
				...
				out = CTxOut(nAmount, scriptPubKey, (TransactionOutType_t)txType, ipcLabel);
				break;
			...
			case 4: //代币登记交易
				...
				//TokenHash Hex串长度必须小于32位（16Byte）
				...
				//对代币交易做输出地址查重
				if (setTokenAddress.count(address))
				{
					bool founded = false;
					multimap<CBitcoinAddress, string>::iterator m;
					m = setTokenAddress.find(address);
					for (uint32_t k = 0; k < setTokenAddress.count(address); k++, m++)
					{
						if (m->second == tokenlabel.getTokenSymbol())
						{
							founded = true;
							break;
						}
					...
					if (founded)
						throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, Token output duplicated address: ") + name_);
				...
				regLabel.totalCount = TokenTotalCountObj.get_int();
				regLabel.value = TokenTotalCountObj.get_int();
				out = CTxOut(nAmount, scriptPubKey, regLabel);
				break;
			...
			case 5: //代币交易
				...
				tokenlabel.value = TokenValueObj.get_int();
				out = CTxOut(nAmount, scriptPubKey, tokenlabel);
				break;

			default:
				printf("Unknown Transaction Type, handled as a normal Tx\n");
			case 0://普通交易
				//if (find_value(o, "devoteCheckBlockContent").isStr())
				//{
				//	devotecontentstring = find_value(o, "devoteCheckBlockContent").get_str();
				//	out = CTxOut(nAmount, scriptPubKey, devotecontentstring);
				//	break;
				//}
				//对普通交易的地址查重
				if (setNormalAddress.count(address))
					throw JSONRPCError(RPC_INVALID_PARAMETER, string("Invalid parameter, normal output duplicated address: ") + name_);
