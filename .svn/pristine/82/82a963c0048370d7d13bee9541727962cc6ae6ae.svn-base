...
{
    {
        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx) || IsFromMe(tx,1))
        {		
		...
		LOCK2(cs_main, cs_wallet);
		{
			...
			while (true)
			{
				...
				if (nChange > 0)
				{
					...
					if (bFindChangScp)
						scriptChange = scriptChangeFind; //Take the first input script and make the change script
	...
	int& nChangePosInOut,  std::string& strFailReason, const CCoinControl* coinControl, bool sign)
{
...

	{
		...
		BOOST_FOREACH(const COutput& coin, vAvailableIPCCoins)
		{
			...
			if (cointxid == txid && coin.i == Index && coin.tx->GetDepthInMainChain() >= DEFAULT_TX_CONFIRM_TARGET)
				isFind = true;
		}
		if (!isFind)
		{
