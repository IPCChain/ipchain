...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, uint32_t blockHeight, uint32_t nPeriodCount, uint64_t  nPeriodStartTime, uint32_t  nTimePeriod, bool fMineWitnessTx)
{
	LogPrintf("[BlockAssembler::CreateNewBlock] begin\n");
	...
	LogPrintf("[BlockAssembler::CreateNewBlock] IN nPeriodCount=%d , nPeriodStartTime=%d , nTimePeriod=%d\n", nPeriodCount, nPeriodStartTime, nTimePeriod);
	...
	//add by xxy
	if (pblock->nTime - pindexPrev->nTime > 20) //与上个块的间隔大于20s则记录下来
		LogPrintfQ(" nHeight = %d 与下个块的打块间隔时间为%d秒  ！\n", pindexPrev->nHeight, pblock->nTime - pindexPrev->nTime);
	//end
  ...
  //  pblocktemplate->vchCoinbaseCommitment = GenerateCoinbaseCommitment(*pblock, pindexPrev, chainparams.GetConsensus());
	...
	if (0 != nRebootHeight)
	{
		if (pindexPrev->nHeight == (nRebootHeight - 1))
		{
			pblock->nNonce = REBOOTLABEL;
		}
	}
