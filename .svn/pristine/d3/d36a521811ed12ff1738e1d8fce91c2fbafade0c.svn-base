...
void RPCExecutor::request(const QString &command)
{
    ...
    catch (UniValue& objError)
    {
        ...
        {   // ShosetClientModelw raw JSON object
...
{
    LogPrintf( "GUI:  RPCConsole::setClientModel \n");
    clientModel = model;
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        LogPrintf( "GUI:  RPCConsole::setClientModel startExecutor\n");
        startExecutor();
    }
    if (!model) {
        LogPrintf( "GUI:  RPCConsole::setClientModel if (!model)\n");
        // Client model is being set to 0, this means shutdown() is about to be called.
        // Make sure we clean up the executor thread
        Q_EMIT stopExecutor();
        thread.wait();
    }
    LogPrintf( "GUI:  RPCConsole::setClientModel end\n");
}
